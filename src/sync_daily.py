...
async def fetch_and_upsert(
    ...
    async with sem:
        ...
        try:
            ...
            label = language_filter if language_filter != "all" else "all"
            print(f"[OK] {label:12s} {path.split('?')[0]:15s} -> {len(data)} ({timeframe}:{period_key})", file=sys.stderr)
            return len(data)
...
async def main():
    ...
    # Build targets: 4 endpoints × (1 overall + 15 languages) = 64 requests
    targets = []
    for ep in CORE_ENDPOINTS:
        targets.append((ep, "all"))
        for lang in SUPPORTED_LANGUAGES:
            targets.append((f"{ep}?language={lang}", lang))
